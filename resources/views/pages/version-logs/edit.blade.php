<x-app-layout>
    @section('title', 'Edit Version Log')
    @if ($errors->any())
    ...
    <div id="kt_app_content" class="app-content flex-column-fluid" style="background-color: #f6f6f6;}">
        ...
        <div id="kt_app_content_container" class="app-container container-fluid" style="padding-left: 0px!important; padding-right: 0px!important">
            <!--begin::Card-->
            <div class="card">
            <form  class="form" action="{{ route('log-version.update', $updateLog->id) }}"
                ...
                <div class="card-body py-4">
                    ...
                    <div class="mb-7">
                        ...
                        <input type="text" name="title" id="title"
                            ...
                            value="{{ old('title' , $updateLog->title) }}" />
                    ...
                    <div class="mb-7">
                        ...
                        <input type="date" name="update_date" id="update_date"
                            ...
                            value="{{ old('update_date',$updateLog->update_date) }}" />
                        <x-input-error class="mt-2" :messages="$errors->get('update_date')" />
                    </div>

                    <div class="mb-7">
                        <label class="fw-semibold fs-6 mb-2">Description update</label>
                        <div id="dynamic-market">
                            @forelse ($updateLogDescriptions as $key => $updateLogDescription)
                                <div class="row border-bottom border-b-1 border-secondary py-2 pb-4" id="row-market-{{$key}}">
                                    <div class="col-7 pt-3">
                                        <div id="description">
                                            <textarea class="form-control form-control-solid" id="description-{{$key}}" name="description[]" placeholder="description">{{ $updateLogDescription->description }}</textarea>
                                            <x-input-error class="mt-2" :messages="$errors->get("description")" />
                                        </div>
                                    </div>
                                    <div class="col-2 pt-3 mb-1 d-flex justify-content-end justify-content-lg-center align-items-end">
                                        <button type="button" class="btn btn-danger btn-sm remove-market" id="{{$key}}">x</button>
                                    </div>
                                </div>
                            @empty
                            @endforelse
...
<script>
    ...
    $('#add-description').click(function() {
        var market = '<div class="row border-bottom border-b-1 border-secondary py-2 pb-4" id="row-market-' + (serviceCounter + 1) + '">' +
                        '<div class="col-7 pt-3">' +
                            '<div id="description">' +
                                '<textarea name="description[]" id="description-' + (serviceCounter + 1) + '" class="form-control form-control-solid mb-3 mb-lg-0 {{ $errors->get("description") ? "is-invalid border border-1 border-danger" : "" }}" placeholder="Description"></textarea>' +
                                '<x-input-error class="mt-2" :messages="$errors->get("description")" />' +
                            '</div>' +
                        '</div>' +
                        '<div class="col-2 pt-3 mb-1 d-flex justify-content-end justify-content-lg-center align-items-end">' +
        ...
        $('#dynamic-market').append(market);
        $('#type-' + (serviceCounter + 1)).select2();
        serviceCounter++; 
    });
